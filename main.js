...
require('./Ramdani.js')
nocache('./Ramdani.js', module => console.log(`${module} telah di update!`))

const starts = async (Ramdani = new WAConnection()) => {
    Ramdani.logger.level = 'warn'
    Ramdani.version = [2,2143,3]
    Ramdani.browserDescription = ['Ramdani Official','Yandex', '3.0']
    console.log(banner)
    Ramdani.on('qr', () => {
...
Ramdani.on("group-participants-update", async (anu) => {
    try {
      ...
      if (anu.action == "add" && mem.includes(Ramdani.user.jid)) {
        Ramdani.sendMessage(anu.jid, "Halo Saya Bot, cara pake bot ketik !help/!menu", "conversation");
      }
